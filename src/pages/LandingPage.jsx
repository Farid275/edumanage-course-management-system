...
const LandingPage = () => {
  ...
  return (
    <>
<div className="min-h-screen w-full bg-[#F8F9FA] overflow-x-hidden">
  {/* Navbar */}
  <header className="bg-surface-container-lowest border-b border-surface-container-highest sticky top-0 z-50 transition-all duration-300 w-full" id="main-nav">
    <div className="w-full max-w-none px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
      <div className="flex items-center gap-2">
        ...
        <span className="font-headline-lg text-headline-lg text-primary font-bold tracking-tight hidden sm:block">EduManage</span>
      </div>
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        <button className="hidden sm:block font-label-md text-label-md text-primary hover:text-primary-container px-4 py-2 transition-colors" onClick={() => navigate("/login")}>Log In</button>
        <button className="bg-tertiary-fixed hover:bg-tertiary-fixed-dim text-on-tertiary-fixed font-label-md text-label-md px-4 py-2 sm:px-6 rounded-lg transition-colors shadow-sm whitespace-nowrap" onClick={() => navigate("/login")}>Get Started</button>
        <button className="md:hidden text-on-surface-variant p-2 hover:bg-surface-container-highest rounded-lg transition-colors">
  ...
  <main className="w-full">
    <section className="relative pt-16 pb-24 md:pt-20 md:pb-32 overflow-hidden bg-surface-container-lowest w-full">
      ...
      <div className="w-full max-w-none px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid w-full grid-cols-1 gap-10 items-center lg:grid-cols-2 xl:gap-16">
          {/* Hero Content */}
          <div className="w-full min-w-0 flex flex-col items-start space-y-6">
            ...
            <h1 className="font-headline-xl text-headline-xl text-primary text-[40px] leading-[48px] md:text-[48px] md:leading-[56px] lg:text-[56px] lg:leading-[64px] font-extrabold max-w-2xl">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 pt-4 w-full sm:w-auto">
              <button className="w-full sm:w-auto justify-center bg-tertiary-fixed hover:bg-tertiary-fixed-dim text-on-tertiary-fixed font-label-md text-label-md px-8 py-3.5 rounded-lg transition-all duration-200 shadow-sm flex items-center gap-2 group" onClick={() => navigate("/login")}>
                Login
                <span className="material-symbols-outlined group-hover:translate-x-1 transition-transform">arrow_forward</span>
              </button>
              <button className="w-full sm:w-auto justify-center bg-transparent border-2 border-primary text-primary hover:bg-primary/5 font-label-md text-label-md px-8 py-3.5 rounded-lg transition-colors flex items-center gap-2" onClick={() => navigate("/admin/dashboard")}>
                View Demo
              </button>
            </div>
            <div className="pt-8 flex flex-wrap items-center gap-4 sm:gap-6 text-on-surface-variant font-label-sm text-label-sm">
          ...
          <div className="w-full min-w-0 relative h-[350px] sm:h-[450px] lg:h-[600px] flex justify-center items-center mt-8 lg:mt-0">
            <div className="absolute inset-0 bg-gradient-to-tr from-surface-container-highest to-surface-container-lowest rounded-2xl shadow-[0_10px_40px_rgba(0,17,58,0.08)] border border-surface-container-highest overflow-hidden transform md:rotate-2 md:scale-105 transition-transform duration-700 hover:rotate-0">
              ...
              <div className="flex h-full w-full bg-surface-container-lowest">
                {/* Sidebar Mockup */}
                <div className="hidden sm:flex w-24 md:w-48 border-r border-surface-container-highest bg-surface-bright flex-col p-4 gap-4">
                ...
                {/* Main Content Mockup */}
                <div className="flex-1 p-4 md:p-6 flex flex-col gap-4 md:gap-6 bg-surface overflow-hidden">
                  {/* Header */}
                  <div className="flex justify-between items-center mb-2">
                    <div className="h-6 md:h-8 w-32 md:w-40 bg-surface-container-highest rounded" />
                    <div className="h-6 md:h-8 w-6 md:w-8 rounded-full bg-primary/20" />
                  </div>
                  {/* Stats Row */}
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                    <div className="h-16 md:h-20 bg-surface-container-lowest rounded-lg border border-surface-container-highest p-3 flex flex-col justify-between gap-2">
                      <div className="h-2 md:h-3 w-1/2 bg-surface-container-highest rounded" />
                      <div className="h-4 md:h-6 w-3/4 bg-primary/10 rounded" />
                    </div>
                    <div className="h-16 md:h-20 bg-surface-container-lowest rounded-lg border border-surface-container-highest p-3 flex flex-col justify-between gap-2">
                      <div className="h-2 md:h-3 w-1/2 bg-surface-container-highest rounded" />
                      <div className="h-4 md:h-6 w-2/3 bg-tertiary-fixed/40 rounded" />
                    </div>
                    <div className="h-16 md:h-20 hidden md:flex bg-surface-container-lowest rounded-lg border border-surface-container-highest p-3 flex-col justify-between gap-2">
                  ...
                  {/* Chart/List Area */}
                  <div className="flex-1 bg-surface-container-lowest rounded-lg border border-surface-container-highest p-4 flex gap-4 min-h-0">
                    <div className="flex-1 flex flex-col gap-3">
                      <div className="h-3 md:h-4 w-24 md:w-32 bg-surface-container-highest rounded mb-2" />
                      <div className="h-6 md:h-8 w-full bg-surface-container rounded" />
                      <div className="h-6 md:h-8 w-full bg-surface-container rounded" />
                      <div className="h-6 md:h-8 w-full bg-surface-container rounded" />
                      <div className="h-6 md:h-8 w-full bg-surface-container rounded" />
                    </div>
    ...
    <section className="py-16 md:py-24 bg-surface w-full" id="features">
      <div className="w-full max-w-none px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12 md:mb-16 max-w-2xl mx-auto">
          <h2 className="font-headline-xl text-headline-xl text-primary mb-4 text-[32px] leading-[40px] md:text-[40px] md:leading-[48px]">Everything you need to run your institution</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 w-full">
          ...
          <div className="bg-surface-container-lowest rounded-xl border border-surface-container-highest p-6 md:p-8 hover:border-primary/30 transition-colors group relative overflow-hidden">
  ...
  {/* Footer */}
  <footer className="bg-surface-container-lowest border-t border-surface-container-highest py-8 md:py-12 w-full">
    <div className="w-full max-w-none px-4 sm:px-6 lg:px-8">
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 md:gap-6">
        ...
        <p className="font-body-sm text-body-sm text-on-surface-variant text-center md:text-left">
          © 2024 EduManage Systems. All rights reserved.
        </p>
