...
import { assignmentsData as initialAssignments } from '../data/dummyData';
...
const AssignmentsManagement = () => {
  ...
  const handleChange = (e) => {
    ...
    setFormData(prev => ({
      ...prev,
      [name]: name === 'submissions' ? parseInt(value) || 0 : value
    }));
  };
  ...
  const filteredAssignments = assignments.filter(a => {
    const matchesSearch = 
      ...
      a.lecturer.toLowerCase().includes(searchQuery.toLowerCase());
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-surface border-b border-surface-container-highest">
                    ...
                    <th className="p-4 font-label-md text-label-md text-on-surface-variant sticky top-0 bg-surface">Submissions</th>
                  ...
                  {filteredAssignments.length === 0 ? (
                    ...
                    filteredAssignments.map(assignment => (
                      <tr key={assignment.id} className="hover:bg-surface-container-lowest/50 transition-colors group">
                        <td className="p-4 font-body-md text-body-md font-medium text-primary">{assignment.title}</td>
                        <td className="p-4 font-body-md text-body-md text-on-surface-variant">{assignment.course}</td>
                        <td className="p-4 font-body-md text-body-md text-on-surface">{assignment.lecturer}</td>
                        <td className="p-4 font-body-md text-body-md text-on-surface">{assignment.due_date}</td>
                        <td className="p-4 font-body-md text-body-md text-on-surface">{assignment.submissions}</td>
                        <td className="p-4">
          ...
          <div className="bg-surface-container-lowest rounded-xl shadow-xl w-full max-w-lg overflow-hidden border border-surface-container-highest animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSave} className="p-6 flex flex-col gap-4 max-h-[80vh] overflow-y-auto">
              <div>
                ...
                <input required type="text" name="title" value={formData.title} onChange={handleChange} className="w-full bg-surface border border-outline-variant rounded-lg px-3 py-2 text-body-md focus:border-secondary-container outline-none" placeholder="e.g. Midterm Essay" />
              </div>
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1">
                  ...
                  <input required type="text" name="lecturer" value={formData.lecturer} onChange={handleChange} className="w-full bg-surface border border-outline-variant rounded-lg px-3 py-2 text-body-md focus:border-secondary-container outline-none" placeholder="e.g. Dr. Turing" />
                </div>
              </div>
